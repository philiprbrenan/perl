#!/usr/bin/perl -I/home/phil/perl/cpan/DataTableText/lib/ -I/home/phil/perl/cpan/GitHubCrud/lib/ -I/home/phil/perl/cpan/DataEditXmlXref/lib/ -I/home/phil/perl/cpan/DitaGBStandard/lib/ -I/home/phil/perl/cpan/FlipFlop/lib/  -I/home/phil/perl/cpan/DitaPCD/lib/
#-------------------------------------------------------------------------------
...
sub pleaseChangeDita {0}                                                        # 1 - process  embedded .pcd (please change dita) files to effect further changes after main conversion to dita
...
sub pcdFolder        {fpd(&home,    qw(pcd))}                                   # Folder containing pcd files
...
sub downloadFromS3                                                              #P Download documents from S3 to the L<downloads|/downloads> folder.
 {if (&download)                                                                # Download if requested
   {lll "Download from S3";
    ...
    for my $f(searchDirectoryTreesForMatchingFiles(downloads, qw(.odp .odt .mtarc)))   # Rename odp and odt files to zip so they get unzipped
...
sub createTarget($$$$)                                                          #P Create source to target description for an input file so we can find where it went
...
sub applyPcds($$)                                                               # Apply pcd if requested
 {my ($if, $x) = @_;                                                            # Inout file, parse tree
  if (pleaseChangeDita)                                                         # Apply user coded changes
   {if (my $blocks = compilePcdFiles(&downloads, &pcdFolder))                   # Compile pcd files
   ...
   {my ($o, $p)  = @_;
    if (couldBeCutOut($o))                                                      # Cut out topics
     {$topicRef  = $o->wrapWith(qq(topicref));                                  # Wrap inner concepts in topicref
      ...
      applyPcds($project->source, $O);                                          # Apply PCDs
...
# Force generation of bookmaps even for single topics.
    if (1 or !$topicRef->isAllBlankText)  ####TEST####Emer4                                           # Topicref has content or overridden by single topic bookmaps
   ...
   {lll "Lint conversion results";
    if (my $report = $lintReport = Data::Edit::Xml::Lint::report                # Lint report
     ...
     {xrefResults;                                                              # Xref results  - xref relies on lint results
      ...
      if (!develop and my $n = &numberOfFiles)                                  # Check number of files if a count has been supplied
  ...
  if (my @results = $ps->finish)                                                # Consolidate results
   {reloadHashes(\@results);                                                    # Recreate attribute methods
    ...
    for my $project(values %$projects)                                          # Each project definition
     {$project->targets or cluck                                                # Check every source file has a target file
