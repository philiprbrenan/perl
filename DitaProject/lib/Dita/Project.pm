...
# project->out is a bad idea in titleToFileName!
...
sub new                                                                         #S Create a project to describe the conversion of one source file containing xml representing documentation into one or more L<Dita|http://docs.oasis-open.org/dita/dita/v1.3/os/part2-tech-content/dita-v1.3-os-part2-tech-content.html> topics.
 {my $p = bless{@_};
  ...
  $p->out //= $p->name // $p->number;                                           # Create an output folder

  return $projects{$n} = $p;

  BEGIN {
    genLValueScalarMethods(q(code));                                            # A code the caller can assign to this project
    genLValueScalarMethods(q(name));                                            # Name of project.
    genLValueScalarMethods(q(number));                                          # Number of the project.
    genLValueScalarMethods(q(out));                                             # Output folder for this project.
...
sub count()                                                                     #S Number of projects
...
our $catalogName;
sub catalogName                                                                 # The Dita Xml catalog to use to validate topics. Assign the file name of the catalog on your computer to this lvalue method.
 {$catalogName // q(/home/phil/r/ge/dita/org.oasis-open.dita.v1_3/dtd/technicalContent/catalog.xml)
 }

our $catalogNameBM;
sub catalogNameBM                                                               # The Dita Xml catalog to use to validate bookmaps. Assign the file name of the catalog on your computer to this lvalue method.
 {$catalogNameBM // q(/home/phil/dita/plugins/org.oasis-open.dita.v1_3/dtd/bookmap/catalog.xml)
 }
...
sub lintTopic($$;$$$)                                                           # Lint a topic by writing the optional xml to a file and invoking xmllint on the file.  If the xml is not specified the current parse tree is printed and so used as the source of the xml.
 {my ($project, $file, $string, $labels, $reusedInProjects) = @_;               # Project, file, optional string of xml else the current parse tree will be printed, optional parse tree containing the label definitions for this xml if any, optional list of projects in which this topic is reused
  ...
  $l->labels  = $labels ? $labels : $project->parse;
  $l->reusedInProject = $reusedInProjects;
  $l->lint;
 } # lintTopic
...
my $emptyTitles = 0;                                                            # Count empty titles
...
sub titleToFileName($$;$)                                                       # Create a unique file name from a title.
 {my ($project, $title, $ext) = @_;                                             # Project, title, desired extension - defaults to .dita
  my $out = $project->out;                                                      # Output folder
  ...
  if ($t =~ m(\A\s*\Z)s)                                                        # Default tile
   {my $n = ++$emptyTitles;
    $t = "SectionWithNoTitle${n}";
   }

  if ($uniqueFileNames{fpe($out, $t, $ext)}++)                                  # Add a number at the end to make the file name unique if the resulting file name is already taken
   {for(1..9999)
     {my $n = sprintf("_%04d", $_);
      my $f = "$t$n";
      my $p = fpe($out, $f, $ext);
      next if $uniqueFileNames{$p}++;
      return $p;
     }
   }
  elsif ($t)                                                                    # The file name was fine from the start
   {return fpe($out, $t, $ext);
   }
...
END
  ...
  is_deeply [matching(qr(1|2))],                                                #TloadFolder #Tmatching
   [bless({ name => 1, number => 1, out => 1, source => "yyy/1.xml" }, "Dita::Project"), #TloadFolder #Tmatching
    bless({ name => 2, number => 2, out => 2, source => "yyy/2.xml" }, "Dita::Project"), #TloadFolder #Tmatching
   ];
...
END2
  ...
  ok project(1)->titleToFileName("title") eq q(1/title.dita);                     #TtitleToFileName
