...
sub topicTypeAndBody($)                                                         #S DITA Topic
 {my ($type) = @_;                                                              # Type - concept; bookmap etc
...
sub Data::Edit::Xml::ditaTopic($)                                               # Create the xml as a DITA topic
 {my ($x) = @_;                                                                 # Parse tree
  my ($n, $N) = topicTypeAndBody($x->tag);
  my $s = -p $x;
  <<END
...
my %uniqueFileNames;                                                            # Ensure we do not create duplicate file names - not safe across multiple processes

sub Data::Edit::Xml::titleToFileName($$;$)                                      # Create a unique file name from a title.
 {my ($node, $ext) = @_;                                                        # Title element, desired extension - defaults to .dita
  my $title = $node->prettyString;                                              # Title element
...
sub Data::Edit::Xml::listToSteps($$;$)                                          # Change B<ol/ul> to B<steps>.
...
sub Data::Edit::Xml::stepsToList($$)                                            # Change B<steps> to B<ol>.
 {my ($o) = @_;                                                                 # Steps node in parse tree
...
sub Data::Edit::Xml::contextFreeConversionsFromDocBookToDita($)                 # Make obvious changes to the parse tree of a DocBook file to make it look more like L<Dita|http://docs.oasis-open.org/dita/dita/v1.3/os/part2-tech-content/dita-v1.3-os-part2-tech-content.html>.
 {my ($x) = @_;                                                                 # Parse tree
  $x->by(sub                                                                    # Do the obvious conversions
   {my ($o) = @_;
