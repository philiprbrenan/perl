...
sub score                                                                       # Respond to a question with a similar answer
 {my ($Q, $A) = @_;                                                             # Question, Answer
  ...
  my %a = map {$_=>1} @a;                                                       # optimization: check that all the words are at least present regardless of order
  return () if grep {!$a{$_}} @q;
...
if ($^O =~ m(bsd|linux)i) {plan tests => 20}                                    # Supported systems
else
 {plan skip_all =>qq(Not supported on: $^O);
 }

is_deeply [score("a d", "a b c"    )], [];
is_deeply [score("a d", "  b c d"  )], [];
