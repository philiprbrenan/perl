...
B<Example:>
  ...
  lll "Test 011";
    ...
    my $y = xref(inputFolder => out, reports => reportFolder);                    # Check results without fixes  # 𝗘𝘅𝗮𝗺𝗽𝗹𝗲
...
If true, add navtitle to outgoing bookmap references to show the title of the target topic.

=head4 changeBadXrefToPh

Change xrefs being placed in B<M3> by L<fixBadRefs> to B<ph>.

=head4 classificationMaps

Create classification maps if true

=head4 deguidize

Set true to replace guids in dita references with file name. Given reference B<g1#g2/id> convert B<g1> to a file name by locating the topic with topicId B<g2>.  This requires the guids to be genuinely unique. SDL guids are thought to be unique by language code but the same topic, translated to a different language might well have the same guid as the original topic with a different language code: =(de|en|es|fr).  If the source is in just one language then the guid uniqueness is a reasonable assumption.  If the conversion can be done in phases by language then the uniqueness of guids is again reasonably assured. L<Data::Edit::Xml::Lint> provides an alternative solution to deguidizing by using labels to record the dita reference in the input corpus for each id encountered, these references can then be resolved in the usual manner by L<Data::Edit::Xml::Lint::relint>.

=head4 deleteUnusedIds

Delete ids (except on topics) that are not referenced in any reference in the corpus regardless of the file component of any such reference.

=head4 fixBadRefs

Fix any remaining bad references after any all allowed attempts have been made to fix failing references by moving the failing reference to the B<xtrf> attribute i.e. placing it in B<M3> possibly renaming the tag to B<ph> if L<changeBadXrefToPh> is in effect as well.

=head4 fixDitaRefs

Fix references in a corpus of L<Dita|http://docs.oasis-open.org/dita/dita/v1.3/os/part2-tech-content/dita-v1.3-os-part2-tech-content.html> documents that have been converted to the L<GB Standard|http://metacpan.org/pod/Dita::GB::Standard> and whose target structure has been written to the named folder.

=head4 fixRelocatedRefs

Fix references to topics that have been moved around in the out folder structure assuming that all file names are unique which they will be if they have been renamed to the GB Standard.

=head4 fixXrefsByTitle

Try to fix invalid xrefs by the Gearhart Title Method enhanced by the Monroe map method if true

=head4 fixedFolder

Fixed files are placed in this folder.

=head4 fixedFolderTemp

Fixed files are placed in this folder if we are on aws but nit the session leader - this folder is then copied back to L<fixedFolder> on the session leader.

=head4 flattenFolder

Files are renamed to the Gearhart standard and placed in this folder if set.  References to the unflattened files are updated to references to the flattened files.  This option will eventually be deprecated as the Dita::GB::Standard is now fully available allowing files to be easily flattened before being processed by Xref.

=head4 getFileUrl

A url to retrieve a specified file from the server running xref used in generating html reports. The complete url is obtained by appending the fully qualified file name to this value.

=head4 html

Generate html version of reports in this folder if supplied

=head4 indexWords

Index words to topics and topics to words if true.

=head4 indexWordsFolder

Folder into which to save words to topic and topics to word indexes if L<indexWords> is true.

=head4 inputFolder

A folder containing the dita and ditamap files to be cross referenced.

=head4 matchTopics

Match topics by title and by vocabulary to the specified confidence level between 0 and 1.  This operation might take some time to complete on a large corpus.

=head4 maxZoomIn

Optional hash of names to regular expressions to look for in each file

=head4 maximumNumberOfProcesses

Maximum number of processes to run in parallel at any one time with a sensible default.

=head4 oxygenProjects

Create oxygen project files for each map - the project file will have an extension of .xpr and the same name and path as the map file or the name return by your implementation of: Data::Edit::Xml::Xref::xprName($map) if present.

=head4 reports

Reports folder: Xref will write text versions of the generated reports to files in this folder.

=head4 requestAttributeNameAndValueCounts

Report attribute name and value counts

=head4 subjectSchemeMap

Create a subject scheme map in the named file

=head4 suppressReferenceChecks

Suppress reference checking - which normally happens by default - but which takes time and might be irrelevant if an earlier xref has already checked all the references.

=head4 validateUrls

Validate urls if true by fetching their headers with L<curl|https://linux.die.net/man/1/curl>
...
=head4 allowUniquePartialMatches

Allow unique partial matches - i.e ignore the stuff to the right of the # in a reference if doing so produces a unique result. This feature has been explicitly disabled for conrefs (PS2-561) and might need to be disabled for other types of reference as well.

=head4 attributeCount

{file}{attribute name} == count of the different xml attributes found in the xml files.

=head4 attributeNamesAndValuesCount

{file}{attribute name}{value} = count

=head4 author

{file} = author of this file.
...
Details of nav titles that were not resolved

=head4 badReferencesCount

The number of bad references at the start of the run - however depending on what options were chosen Xref might ameliorate these bad references and thereby reduce this count.

=head4 badTables

Array of tables that need fixing.

=head4 badXml1

[Files] with a bad xml encoding header on the first line.

=head4 badXml2

[Files] with a bad xml doc type on the second line.

=head4 baseFiles

{base of file name}{full file name}++ Current location of the file via uniqueness guaranteed by the GB standard

=head4 baseTag

Base Tag for each file

=head4 bookMapRefs

{bookmap full file name}{href}{navTitle}++ References from bookmaps to topics via appendix, chapter, bookmapref.

=head4 conRefs

{file}{href}{tag}++ : conref source detail

=head4 createReports1

Reports requested before references fixed

=head4 createReports2

Reports requested after references fixed

=head4 currentFolder

The current working folder used to make absolute file names from relative ones

=head4 docType

{file} == docType:  the docType for each xml file.

=head4 duplicateIds

[file, id]     Duplicate id definitions within each file.

=head4 duplicateTopicIds

[topicId, [files]] Files with duplicate topic ids - the id on the outermost tag.

=head4 emptyTopics

{file} : topics where the *body is empty.

=head4 errors

Number of significant errors as reported in L<statusLine> or 0 if no such errors found

=head4 exteriorMaps

{exterior map} : maps that are not referenced by another map

=head4 fileExtensions

Default file extensions to load

=head4 fixRefs

{file}{ref} where the href or conref target is not valid.

=head4 fixedRefsBad

[] hrefs and conrefs from L<fixRefs|/fixRefs> which were moved to the "xtrf" attribute as requested by the L<fixBadHrefs|/fixBadHrefs> attribute because the reference was invalid and could not be improved by L<deguidization|/deguidize>.

=head4 fixedRefsGB

[] files fixed to the Gearhart-Brenan file naming standard

=head4 fixedRefsGood

[] hrefs and conrefs from L<fixRefs|/fixRefs> which were invalid but have been fixed by L<deguidizing|/deguidize> them to a valid file name.

=head4 fixedRefsNoAction

[] hrefs and conrefs from L<fixRefs|/fixRefs> for which no action was taken.

=head4 flattenFiles

{old full file name} = file renamed to Gearhart-Brenan file naming standard

=head4 goodImageFiles

{file}++ : number of references to each good image

=head4 goodNavTitles

Details of nav titles that were resolved.

=head4 guidHrefs

{file}{href} = location where href starts with GUID- and is thus probably a guid.

=head4 guidToFile

{topic id which is a guid} = file defining topic id.

=head4 hrefUrlEncoding

Hrefs that need url encoding because they contain white space.

=head4 idNotReferenced

{file}{id}++ - id in a file that is not referenced

=head4 idReferencedCount

{file}{id}++ - the number of times this id in this file is referenced from the rest of the corpus

=head4 idTags

{file}{id}[tag] The tags associated with each id in a file - there might be more than one if the id is duplicated

=head4 ids

{file}{id}   - id definitions across all files.

=head4 idsRemoved

{id}++ : Ids removed from all files

=head4 images

{file}{href}   Count of image references in each file.

=head4 imagesReferencedFromBookMaps

{bookmap full file name}{full name of image referenced from topic referenced from bookmap}++

=head4 imagesReferencedFromTopics

{topic full file name}{full name of image referenced from topic}++

=head4 imagesToRefferingBookMaps

{image full file name}{bookmap full file name}++ : images to referring bookmaps

=head4 indexedWords

{word}{full file name of topic the words occurs in}.

=head4 inputFileToTargetTopics

{input file}{target file}++ : Tells us the topics an input file was split into

=head4 inputFiles

Input files from L<inputFolder|/inputFolder>.

=head4 inputFolderImages

{full image file name} for all files in input folder thus including any images resent

=head4 ltgt

{text between &lt; and &gt}{filename} = count giving the count of text items found between &lt; and &gt;

=head4 maxZoomOut

Results from L<maxZoomIn|/maxZoomIn>  where {file name}{regular expression key name in L<maxZoomIn|/maxZoomIn>}++

=head4 md5Sum

MD5 sum for each input file.

=head4 md5SumDuplicates

{md5sum}{file}++ : md5 sums with more than one file

=head4 missingImageFiles

[file, href] == Missing images in each file.

=head4 missingTopicIds

Missing topic ids.

=head4 noHref

Tags that should have an href but do not have one.

=head4 notReferenced

{file name} Files in input area that are not referenced by a conref, image, bookmapref or xref tag and are not a bookmap.

=head4 olBody

The number of ol under body by file

=head4 originalSourceFileAndIdToNewFile

{original file}{id} = new file: Record mapping from original source file and id to the new file containing the id

=head4 otherMeta

{original file}{othermeta name}{othermeta content}++ : the contents of the other meta tags

=head4 otherMetaBookMapsAfterTopicIncludes

Bookmap othermeta after  topic othermeta has been included

=head4 otherMetaBookMapsBeforeTopicIncludes

Bookmap othermeta before topic othermeta has been included

=head4 otherMetaConsolidated

{Name}{Content}++ : consolidated other meta data across entire corpus

=head4 otherMetaDuplicatesCombined

Duplicate othermeta in bookmaps with called topics othermeta included

=head4 otherMetaDuplicatesSeparately

Duplicate othermeta in bookmaps and topics considered separately

=head4 otherMetaPushToBookMap

Othermeta that can be pushed to the calling book map

=head4 otherMetaRemainWithTopic

Othermeta that must stay in the topic

=head4 parseFailed

{file} files that failed to parse.

=head4 publicId

{file} = Public id on Doctype

=head4 references

{file}{reference}++ - the various references encountered

=head4 relocatedReferencesFailed

Failing references that were not fixed by relocation

=head4 relocatedReferencesFixed

Relocated references fixed

=head4 requiredCleanUp

{full file name}{cleanup} = number of required-cleanups

=head4 results

Summary of results table.

=head4 sourceTopicToTargetBookMap

{input topic cut into multiple pieces} = output bookmap representing pieces

=head4 statusLine

Status line summarizing the cross reference.

=head4 statusTable

Status table summarizing the cross reference.

=head4 tableDimensions

{file}{columns}{rows} == count

=head4 tagCount

{file}{tags} == count of the different tag names found in the xml files.

=head4 tags

Number of tags encountered

=head4 tagsTextsRatio

Ratio of tags to text encountered

=head4 targetFolderContent

{file} = bookmap file name : the target folder content which shows us where an input file went

=head4 targetTopicToInputFiles

{current file} = the source file from which the current file was obtained

=head4 texts

Number of texts encountered
...
{full file name} = title of file.

=head4 titleToFile

{title}{file}++ if L<fixXrefsByTitle> is in effect

=head4 topicFlattening

{topic}{sources}++ : the source files for each topic that was flattened

=head4 topicFlatteningFactor

Topic flattening factor - higher is better

=head4 topicIds

{file} = topic id - the id on the outermost tag.

=head4 topicsFlattened

Number of topics flattened

=head4 topicsNotReferencedFromBookMaps

{topic file not referenced from any bookmap} = 1

=head4 topicsReferencedFromBookMaps

{bookmap full file name}{topic full file name}++ : bookmaps to topics

=head4 topicsToReferringBookMaps

{topic full file name}{bookmap full file name}++ : topics to referring bookmaps

=head4 urls

{topic full file name}{url}++ : urls found in each file

=head4 urlsBad

{url}{topic full file name}++ : failing urls found in each file

=head4 urlsGood

{url}{topic full file name}++ : passing urls found in each file

=head4 validationErrors

True means that Lint detected errors in the xml contained in the file.

=head4 vocabulary

The text of each topic shorn of attributes for vocabulary comparison.

=head4 xRefs

{file}{href}++ Xrefs references.

=head4 xrefBadFormat

External xrefs with no format=html.

=head4 xrefBadScope

External xrefs with no scope=external.

=head4 xrefsFixedByTitle

Xrefs fixed by locating a matching topic title from their text content.
